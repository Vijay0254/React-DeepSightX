...
const EyeResult = ({ eye, diagnosis, accuracy }) => {
  ...
  const barColor = Math.floor(accuracy * 100) >= 70 ? '#22c55e' : Math.floor(accuracy * 100) >= 40 ? '#facc15' : '#ef4444'
  ...
  return (
    <div className='border lg:w-[50%] shadow-xl bg-white border-slate-300 rounded-2xl p-7'>
      ...
      <div className='space-y-2 my-4'>
        <p className='text-lg font-medium'>Accuracy: <span className='text-primary pl-1'>{Math.floor(accuracy * 100)}%</span></p>
        <div className='w-full h-3.5 bg-slate-300 rounded-full relative'>
          <p style={{width: `${Math.floor(accuracy * 100)}%`, backgroundColor: barColor}} className={`absolute left-0 rounded-full top-0 bottom-0`}></p>
        </div>
      </div>
